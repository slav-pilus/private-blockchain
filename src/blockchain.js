...
class Blockchain {
    ...
    _addBlock(block) {
        ...
        return new Promise(async (resolve, reject) => {
            await this.validateChain().then(result => {
            ...
            console.log("adding hash: " + block.hash);
    ...
    submitStar(address, message, signature, star) {
        ...
        return new Promise(async (resolve, reject) => {
            ...
            if(currentTime - messageTime > 60 * 5 || !validMessage) {
    ...
    validateChain() {
        ...
        return new Promise(async (resolve, reject) => {
            ...
            for (const [i, block] of self.chain.entries()) {
                    //check integrity of each block
                    await block.validate().then(result => {
                        if (result === false) {
                            errorLog.push({"block": block.height})
                        }
                    }).catch(err => {
                        reject(Error(err))
                    });
